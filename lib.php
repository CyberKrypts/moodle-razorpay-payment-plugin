...
class enrol_razorpay_plugin extends enrol_plugin {
    ...
    public function add_course_navigation($instancesnode, stdClass $instance) {
        if ($instance->enrol !== 'razorpay') {
             throw new coding_exception('Invalid enrol instance type!');
        }

        $context = context_course::instance($instance->courseid);
        if (has_capability('enrol/razorpay:config', $context)) {
            $managelink = new moodle_url('/enrol/razorpay/edit.php',
    ...
    public function get_action_icons(stdClass $instance) {
        ...
        if (has_capability('enrol/razorpay:config', $context)) {
            $editlink = new moodle_url("/enrol/razorpay/edit.php",
            array('courseid' => $instance->courseid, 'id' => $instance->id));
    ...
    public function get_newinstance_link($courseid) {
        ...
        return new moodle_url('/enrol/razorpay/edit.php', array('courseid' => $courseid));
    }
    ...
    public function enrol_page_hook(stdClass $instance) {
        ...
        $enrolstatus = $this->can_razorpay_enrol($instance);

        if (!$enrolstatus) {
            return get_string('maxenrolledreached', 'enrol_razorpay');
        }
        ...
        if (abs($cost) < 0.01) { // No cost, other enrolment methods (instances) should be used.
            ...
            if (isguestuser()) { // Force login only for guest user, not real users with guest role.
                ...
                include($CFG->dirroot.'/enrol/razorpay/enrol.php');
            }
     ...
     * @return string html text, usually a form in a text box
     */
    public function can_razorpay_enrol(stdClass $instance) {
        ...
        if ($instance->customint3 > 0) {
            ...
            if ($count >= $instance->customint3) {
                // Bad luck, no more razorpay enrolments here.
                return false;
            }
        }
        return true;
    ...
    public function get_user_enrolment_actions(course_enrolment_manager $manager, $ue) {
        ...
        $params['ue'] = $ue->id;
        if ($this->allow_unenrol($instance) && has_capability("enrol/razorpay:unenrol", $context)) {
        ...
        if ($this->allow_manage($instance) && has_capability("enrol/razorpay:manage", $context)) {
